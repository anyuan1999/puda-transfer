#!/bin/bash
# PUDA Cross-Domain Transfer: orthrus × {S1, S2} only
# Single-GPU (GPU1) so GPU0 can be used for other work later.
#
# Container deadline: 2026-05-29 23:00. Persistence trigger at 22:00.
#
# Usage:
#   nohup bash run_orthrus_s1_s2.sh > artifacts/run_orthrus_s1_s2.log 2>&1 &
#
...
# ----------------------------------------------------------------------
# Phase 0: wait for trace_e3 DB to be ready
# (pg_restore 可能还在另一个 shell 里继续，等它结束)
# ----------------------------------------------------------------------
echo
echo "===== PHASE 0: wait for trace_e3 pg_restore to finish ====="
while pgrep -f "pg_restore.*trace_e3" > /dev/null; do
    ...
    echo "[$(date '+%H:%M:%S')] pg_restore still running, event_table rows: ${rows:-(empty)}"
    sleep 60
done
echo "[$(date '+%H:%M:%S')] pg_restore DONE"
su - pgsql -c "PATH=/root/miniconda3/envs/pids/bin:\$PATH psql -h 127.0.0.1 -U pgsql -d trace_e3 -c '\\dt+'" 2>&1 | tail -8

# ----------------------------------------------------------------------
# Phase 1: train orthrus on TRACE_E3
# ----------------------------------------------------------------------
echo
...
echo
echo "================================================================"
echo "# RUN orthrus×{S1,S2}  done: $(date)"
echo "# results CSV (S1+S2 rows for orthrus):"
grep -E "^orthrus,(S1|S2)," "$RESULTS"
echo "================================================================"
